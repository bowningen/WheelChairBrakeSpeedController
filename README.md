...
※1 : 研究時間が本当になかったので未実装のままです。(v1.1.0で実装予定)


※2 : Raspberry Piを用いて制御する場合に変数に用いる名前として指定します。ただRaspberry Pi側でのモーターの動作の実装はしておりません(v1.2.0で実装予定)


※1 : Raspberry Piを用いて制御する場合に出力ピンとして指定します。ただRaspberry Pi側でのモーターの動作の実装はしておりません(v1.2.0で実装予定)
