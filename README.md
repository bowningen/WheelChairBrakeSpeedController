...
#### v1と比較しておすすめできるポイント
・自動で読んできたホール電圧より処理に使用する変数の値を変化させますので、磁場が強いような場所でも煩わしい設定無しにご使用いただくことが可能です。
・
...
#### 環境に合わせて設定していただく必要がある可能性があるものは、変数名の右上に「 * 」をつけております
...
| A_TIME | プログラム実行からの経過時間(us)(unsigned long) | 0 |
| D_TIME1 | ホール素子1用のdeltaTime格納変数(us)(unsigned long) | 0 |
| D_TIME2 | ホール素子2用のdeltaTime格納変数(us)(unsigned long) | 0 |
| C_TIME | 処理時間を考慮して左右の誤差を減らすための時間格納変数(us)(unsigned long) | 0 |
| SP_N | 現在の物体の速度(km/h)格納変数(float) | 0.00 |
| SP_N_LAST | 誤検出対策に使用するひとつ前の物体の速度格納変数1(float) | 0.00 |
| SP_N_LAST2 | 誤検出対策に使用するひとつ前の物体の速度格納変数2(float) | 0.00 |
| SP_N_CK * | おかしい値が連続して出てしまっているときに自動リセットをかけるための速度格納配列(float)(※5) | len(SP_N_CK) = 10 |
| SP_N_PUSH | SP_N_CKに基づいて処理する際のスタック(float) | 0.00 |
| TIRE * | 速度を計測するタイヤ(あるいは円盤)が何インチであるかの指定(float) | 16.0 |
| MAG_NUM * | 速度を計測するタイヤ(あるいは円盤)につけた磁石の数の指定(int) | 4 |
| DIST | TIMEとMAG_NUMから各磁石間の長さを出してきて格納する変数(float) | 0.00 |
| VTE_OFF * | 速度を計測するタイヤ(あるいは円盤)が何インチであるか | 16 |
| VTE_ERROR * | 速度を計測するタイヤ(あるいは円盤)につけた磁石の数 | 4 |
| IR * | R側ホール素子の初期化処理ON/OFF指定(自動で磁石接近時と離れている時の電圧を再設定します) | 0(0:ON/1:OFF) |
| IL * | L側ホール素子の初期化処理ON/OFF指定(IRと同様) | 0(0:ON/1:OFF) |
| ct | 誤検知対策で、一度磁石が接近した状態から数ループだけ検出しない処理を挟む...その回数記録変数(int) | 5(while文内) |
| CC_L | 磁石が離れた状態の電圧と現在電圧との差の絶対値を示す変数(L側)(float) | 0.00 |
| CC_R | 磁石が離れた状態の電圧と現在電圧との差の絶対値を示す変数(R側)(float) | 0.00 |
| DeltaT | 前回磁石が接近した時からの経過時間を示す変数(float) | 0.00 |
| wl | (※未使用の変数です) | 1 |
