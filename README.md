...
### v2
・なし
(stdio.hとstdlib.hをインクルードしている行をコメントアウトしていますが、恐らく使用したボード固有のものか何かで、書いてなくてもエラーはないことからなくても大丈夫だと思われます。)
...
※実行時にエラーが出る場合が多いですが、シリアル通信関連で例外が発生していることがほとんどなので、何度かCtrl+Zで強制終了して再実行を繰り返してください。（v1.1.0で修正予定）
...
現在Raspberry Pi側でのモーターの動作の実装はしておりません(Arduinoを使用してください)(v1.2.0で実装予定)
...
| 変数名                 | 役割                                      | デフォルト値                       |
| ---------------------- | ----------------------------------------- | ---------------------------------- |
| HOLE1_POW               | ホール素子_1用の電源出力ピンの設定 | 12                       |
| HOLE2_POW               | ホール素子_2用の電源出力ピンの設定 | 12                        |
| HOLE1_READ                 | 各種情報の出力先(未使用) (※1)                           | result.txt                         |
| HOLE2_READ                  | ホール素子1つ目の監視ピン(未使用)                         | 15                                 |
| R_F                  | ホール素子2つ目の監視ピン(未使用)                         | 16                                 |
| R_R                  | モーターの名前1 (※2)                                   | L                                  |
| L_F                  | モーターの名前2 (※2)                                   | R                                  |
| L_R                | モーター制御信号出力ピン1 (※3)                          |  7                                 |
| MODE_SW                | モーター制御信号出力ピン1 (※3)                          | 11                                 |
| R_MOTOR_OUT1                   | 速度を計測するタイヤ(あるいは円盤)が何インチであるか         | 16                                 |
| R_MOTOR_OUT2                | 速度を計測するタイヤ(あるいは円盤)につけた磁石の数          | 4                                  |
| L_MOTOR_OUT1                | 磁石が接近していない状態でのホール電圧                     | 2.28                               |
| L_MOTOR_OUT2              | 磁石が接近していない状態での許容される電圧のブレの範囲       | 0.2                                |
| RETURN                | ブレーキOFF状態でのモーターの角度(未使用) (※4)            | 80                                 |
| STOP                | フルブレーキ状態でのモーターの角度(未使用) (※4)           | 120                                |
| HOLE1_V               | ブレーキOFF状態でのモーター1の角度                        | 90                                 |
| HOLE2_V               | フルブレーキ状態でのモーター1の角度                       | 110                                |
| HOLEx_V               | ブレーキOFF状態でのモーター2の角度                        | 95                                 |
...
ただこのエラーは、無視してもその一瞬だけ電圧が読めていないだけでそのままプログラムは進行します
...
とはいえある程度の精度はありますので、「磁石が接近していない時の電圧(VTE_OFF)」「誤差として切り捨てて磁石が接近していないとする値(VTE_ERROR)(VTE_OFFと現在電圧の差分がこれ以下なら誤差とします)」等各種定数の値をお使いの環境に合わせたものにしていただければ、ある程度の速度を算出することは可能なはずです。
設定をご確認ください。
